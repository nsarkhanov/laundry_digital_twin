...
export default function ConfigurationDialog({
    ...
    const [newIroningMachine, setNewIroningMachine] = useState({
        model: '', ironing_labor_hours: 10.0, energy_consumption_kwh_per_hour: 3.0
    });
    ...
    const handleAddIroningMachine = async () => {
        ...
        if (await actions.addIroningMachine(newIroningMachine)) {
            setNewIroningMachine({ model: '', ironing_labor_hours: 10.0, energy_consumption_kwh_per_hour: 3.0 });
    ...
    // Request delete confirmation - shows dialog instead of deleting immediately
    ...
    const confirmDelete = async () => {
        ...
        switch (type) {
            case 'location':
                success = await actions.deleteLocation(id);
                if (success && config.locationId === id) {
                    setConfig(prev => ({ ...prev, locationId: null }));
                }
                break;
            case 'washingMachine':
                success = await actions.deleteWashingMachine(id);
                if (success && config.washingMachineId === id) {
                    setConfig(prev => ({ ...prev, washingMachineId: null }));
                }
                break;
            case 'dryingMachine':
                success = await actions.deleteDryingMachine(id);
                if (success && config.dryingMachineId === id) {
                    setConfig(prev => ({ ...prev, dryingMachineId: null }));
                }
                break;
            case 'ironingMachine':
                success = await actions.deleteIroningMachine(id);
                if (success && config.ironingMachineId === id) {
                    setConfig(prev => ({ ...prev, ironingMachineId: null }));
                }
                break;
            case 'chemical':
                success = await actions.deleteChemical(id);
                if (success) {
    ...
    // Wrapper functions for easier calling from components
    const handleDeleteLocation = (id) => {
        const loc = locations.find(l => l.id === id);
        requestDelete('location', id, loc?.name || 'this location');
    };

    const handleDeleteWashingMachine = (id) => {
        const machine = washingMachines.find(m => m.id === id);
        requestDelete('washingMachine', id, machine?.model || 'this washing machine');
    };

    const handleDeleteDryingMachine = (id) => {
        const machine = dryingMachines.find(m => m.id === id);
        requestDelete('dryingMachine', id, machine?.model || 'this drying machine');
    };

    const handleDeleteIroningMachine = (id) => {
        const machine = ironingMachines.find(m => m.id === id);
        requestDelete('ironingMachine', id, machine?.model || 'this ironing machine');
    };

    const handleDeleteChemical = (id) => {
        const chem = chemicals.find(c => c.id === id);
        requestDelete('chemical', id, chem?.name || 'this chemical');
    };
        ...
        <>
                ...
                <DialogContent className="bg-[#18181b] border-white/10 max-w-5xl max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="py-2">
                        <Tabs defaultValue="tariffs" className="w-full">
                            <TabsList className="grid w-full grid-cols-4 bg-white/5 p-1 rounded-xl mb-6">
                                ...
                                <TabsTrigger value="operations" className="rounded-lg data-[state=active]:bg-white/10 data-[state=active]:text-white">
            ...
            <AlertDialog open={deleteConfirm.open} onOpenChange={(open) => !open && setDeleteConfirm({ open: false, type: '', id: '', name: '' })}>
                <AlertDialogContent className="bg-[#18181b] border-white/10">
                    <AlertDialogHeader>
                        <AlertDialogTitle className="text-white">Delete Confirmation</AlertDialogTitle>
                        <AlertDialogDescription>
            ...
            <Dialog open={showLocationDialog} onOpenChange={setShowLocationDialog}>
                <DialogContent className="bg-[#18181b] border-white/10">
                    ...
                    <DialogFooter>
                        <Button onClick={handleAddLocation} className="bg-amber text-black hover:bg-amber/90">
                            Save Location
                        </Button>
                    </DialogFooter>
                </DialogContent>
